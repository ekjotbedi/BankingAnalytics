...
from sklearn.preprocessing import StandardScaler    # normalizes features before clustering
...
# The 8 features fed into the clustering model capture volume, frequency, health, engagement, recency, and seasonality.
FEATURE_COLS = [
    "avg_monthly_volume",      # how much $ moves per month
    "avg_monthly_txn_count",   # how often they transact
    "net_flow",                # revenue minus expenses
    "credit_debit_ratio",      # incoming vs outgoing ratio
    "num_categories",          # breadth of banking product usage
    "days_since_last_txn",     # recency — high value
...
SEGMENT_LABELS = {
    0: "High-Growth",    # high volume, positive net flow
...
def scale_features(df: pd.DataFrame):
    ...
    would completely dominate num_categories, giving meaningless clusters.
    """
    X = df[FEATURE_COLS].fillna(0).values # fills any remaining nulls
...
def pca_reduce(X_scaled: np.ndarray):
    """
    Reduces the 8 features  to 2 principal components for the scatter plot.
    PCA finds the two directions in the data that capture the most variance,
    so that the 2D plot shows the natural groupings.
    """
...
def find_optimal_k(X_scaled: np.ndarray, k_range=range(2, 10)) -> int:
    ...
    Try K-Means for each K from 2 to 9 and record:
      ...
      - Silhouette score — how distinct the clusters are
    ...
    scores_df.to_csv(ELBOW_CSV, index=False)
...
def fit_kmeans(X_scaled: np.ndarray, k: int) -> np.ndarray:
    ...
    """
    km = KMeans(n_clusters=k, random_state=42, n_init=10) #n_init=10 runs 10 different random starting points and picks the best
    labels = km.fit_predict(X_scaled)
